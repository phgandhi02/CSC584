...
void Boid::update(float dt)
{
    ...
    if (!m_target.operator==(noTarget))
    {
        // set the target for the controller equal to the target from the input handler (ie. mouse)
        controller->target = m_target;
    }
    ...
    {
        // check for collisions with window border
        ...
        // get pos and orient vars for setting behavior if collision happened.
        auto boid_pos = m_character.getPosition();
        auto boid_orient = m_character.getOrientation();

        // Flee from window border if collision occured
        if ((left_collision | right_collision | up_collision | down_collision))
        {
            ...
            // override the previous steering behavior with flee for this update call.
...
void Boid::update(float dt, std::array<std::array<Cell, MAP_WIDTH>, MAP_HEIGHT> map)
{
    // Check if boid has a valid steering behavior attached.
    if (controller == nullptr)
    {
    ...
    Static noTarget = Static();
    if (mouseInputOn | m_target.operator==(noTarget))
    {
        // Assign target from mouse click using input handler object.
        auto target = m_inputHandler.update(); // update will return a Static object with null_output false;
        // set the target for the controller equal to the target from the input handler (ie. mouse)
    ...
    if (m_steering.null_output == false)
    {
        ...
        // sf::Vector2f cell_global_position;
        ...
        //         if (cell == Cell::Wall)
        //         {
        //             cell_global_position = sf::Vector2f(static_cast<float>(CELL_SIZE * j), static_cast<float>(CELL_SIZE * i));
        //         };
        //     }
        // }
        // Update the position and orientation
